...
public class CooperationAdapter extends BaseQuickAdapter<PartnerBean.ListBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, PartnerBean.ListBean item) {
        ...
        helper.setText(R.id.tv_company, Config.getConfig().getBusinessName(item.getBusinessOneCode()));
        if (item.getStatus() != 0) {
            helper.setText(R.id.btn_unconfirm, Config.getConfig().getConstBean().getShopConstant()
                    .get(Constant.COOPERATION_STATUS).get(item.getStatus()));
        } else {
        ...
        helper.setText(R.id.tv_bugone, Config.getConfig().getBusinessName(item.getBusinessOneCode()));
        helper.setText(R.id.tv_time_limit, item.getBeginTime()
                + "  到  " + item.getEndTime());
        helper.setText(R.id.tv_repair_install, Config.getConfig().getConstBean().getShopConstant()
                .get(Constant.COOPERATION_TYPE).get(item.getBusType()));
