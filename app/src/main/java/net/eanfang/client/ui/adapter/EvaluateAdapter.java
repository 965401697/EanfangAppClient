...
public class EvaluateAdapter extends BaseQuickAdapter<ReceivedEvaluateBean.ListBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, ReceivedEvaluateBean.ListBean item) {
        ...
        if (!StringUtils.isEmpty(item.getCreatUser().getAccountEntity().getAvatar())) {
            iv_header.setImageURI(Uri.parse(item.getCreatUser().getAccountEntity().getAvatar()));
        }

        int totalStar = item.getItem1() + item.getItem1() + item.getItem2() +
                item.getItem3() + item.getItem4() + item.getItem5();
        int average = totalStar / 5;

        helper.setText(R.id.tv_name, item.getCreatUser().getAccountEntity().getRealName())
