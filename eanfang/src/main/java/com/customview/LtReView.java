 ...
 * 2.密码的显示和隐藏
 */
public class LtReView extends android.support.v7.widget.AppCompatEditText {
    /**
     * TODO 左侧的文字内容，没有则不设置
     */
    ...
    public LtReView(Context context) {
        this(context, null);
    }

    public LtReView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.editTextStyle);
    }

    public LtReView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initData(context, attrs, defStyleAttr);
    }

    private void initData(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.LtReView, defStyleAttr, 0);
        leftText = typeArray.getString(R.styleable.LtReView_leftText);
        leftTextColor = typeArray.getColor(R.styleable.LtReView_leftTextColor, Color.BLACK);
        leftTextSize = typeArray.getDimension(R.styleable.LtReView_leftTextSize, 14);
        clickType = typeArray.getInt(R.styleable.LtReView_rightButtonClickType, 0);
        isShowDownLine = typeArray.getBoolean(R.styleable.LtReView_isShowDownLine, true);
        leftTextPadding = typeArray.getInt(R.styleable.LtReView_leftTextPadding, dip2px(8));
        downLineDefaultColor = typeArray.getColor(R.styleable.LtReView_downLineDefaultColor, Color.GRAY);
        downLineFocusColor = typeArray.getColor(R.styleable.LtReView_downLineFocusColor, Color.LTGRAY);
        typeArray.recycle();
        setBackground(null);
        initPaint();
    }
    ...
    private void dealRightButtonClickType() {
        ...
        if (clickType == 0) {
            ...
            setCompoundDrawables(getCompoundDrawables()[0], getCompoundDrawables()[1], isShowPass ? getDrawable(R.mipmap.ic_launcher) : getDrawable(R.mipmap.ic_launcher), getCompoundDrawables()[3]);
        }
    }
