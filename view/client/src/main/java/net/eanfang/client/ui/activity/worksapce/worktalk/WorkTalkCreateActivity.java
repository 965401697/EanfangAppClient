...
public class WorkTalkCreateActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        mCompanyId = ClientApplication.get().getLoginBean().getAccount().getDefaultUser().getCompanyEntity().getOrgId();
        mTopCompanyId = ClientApplication.get().getLoginBean().getAccount().getDefaultUser().getCompanyEntity().getTopCompanyId();
        tv_company_name.setText(ClientApplication.get().getLoginBean().getAccount().getDefaultUser().getCompanyEntity().getOrgName());
...
//            }
            ...
            if (newPresonList.size() == 0) {
                //工作协同默认值
                workTalkDetailBean.setAssigneeUserId(String.valueOf(ClientApplication.get().getUserId()));
                workTalkDetailBean.setAssigneeOrgCode(ClientApplication.get().getOrgCode());
                workTalkDetailBean.setAssigneeCompanyId(String.valueOf(ClientApplication.get().getCompanyId()));
                workTalkDetailBean.setAssigneeTopCompanyId(String.valueOf(ClientApplication.get().getTopCompanyId()));
            } else {
                //工作协同默认值
