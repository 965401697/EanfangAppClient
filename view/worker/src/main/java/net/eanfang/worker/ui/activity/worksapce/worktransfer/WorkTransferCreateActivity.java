...
public class WorkTransferCreateActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        OrgEntity orgEntity=userEntity.getCompanyEntity();
        OrgEntity deper=userEntity.getDepartmentEntity();
        mUserId = userEntity.getUserId();
        mCompanyId = orgEntity.getOrgId();
        mTopCompanyId = orgEntity.getTopCompanyId();
        mDepartmentId = orgEntity.getOrgCode();
        tvCompanyName.setText(orgEntity.getOrgName());
        if (deper != null) {
            tvDepartmentName.setText(deper.getOrgName());
        }
...
//            workTransferDetailBean.setAssigneeOrgCode(mReceiverDeparrmentID);
            ...
            if (newPresonList.size() == 0) {
                //工作协同默认值
                workTransferDetailBean.setAssigneeUserId(String.valueOf(WorkerApplication.get().getUserId()));
                workTransferDetailBean.setAssigneeOrgCode(WorkerApplication.get().getOrgCode());
                workTransferDetailBean.setAssigneeCompanyId(String.valueOf(WorkerApplication.getApplication().getCompanyId()));
                workTransferDetailBean.setAssigneeTopCompanyId(String.valueOf(WorkerApplication.getApplication().getTopCompanyId()));
            } else {
                //工作协同默认值
