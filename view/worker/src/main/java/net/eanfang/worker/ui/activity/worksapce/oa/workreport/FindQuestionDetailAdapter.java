...
//                helper.addOnClickListener(R.id.tv_delete);
    ...
    private void initPic(BaseViewHolder helper, WorkReportInfoBean.WorkReportDetailsBean item) {
        if (!StrUtil.isEmpty(item.getPictures())) {
            ...
            if (urls.length >= 1) {
                GlideUtil.intoImageView(mComtext,BuildConfig.OSS_SERVER + Uri.parse(urls[0]),helper.getView(R.id.iv_pic1));
            ...
            if (urls.length >= 2) {
                GlideUtil.intoImageView(mComtext,BuildConfig.OSS_SERVER + Uri.parse(urls[1]),helper.getView(R.id.iv_pic2));
            ...
            if (urls.length >= 3) {
                GlideUtil.intoImageView(mComtext,BuildConfig.OSS_SERVER + Uri.parse(urls[2]),helper.getView(R.id.iv_pic3));
